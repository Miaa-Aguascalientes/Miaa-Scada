...
for id_p, info in mapa_pozos_dict.items():
    ...
    if es_falla_com:
        ...
        q_val, p_val = data_scada.get(info['caudal'], (0, "N/A"))[0], data_scada.get(info['presion'], (0, "N/A"))[0]
        if val_bba == 1:
...
# --- LÓGICA DE DETALLE SECTOR (Solo si hay parámetro en URL) ---
...
with col_capas:
    st.markdown("### 🗺️ Capas")
    ver_sectores, ver_pozos, ver_etiquetas = st.checkbox("Sectores", True), st.checkbox("Pozos", True), st.checkbox("ID Pozos", True)
...
with col_mapa:
    ...
    def get_blink_icon(color):
        return f'<div style="width:8px; height:8px; background:{color}; border-radius:50%; box-shadow:0 0 8px {color}; animation:blinker 1s linear infinite;"></div><style>@keyframes blinker {{ 50% {{ opacity: 0.2; }} }}</style>'

    if ver_sectores:
        for s in sectores:
            url_s = f"./?sector_id={urllib.parse.quote(str(s['sector']))}"
            pop_s = f'<div style="background:#0b1a29; padding:10px; border-radius:5px; border:1px solid #00d4ff;"><h4 style="color:#00d4ff;">Sector: {s["sector"]}</h4><hr><a href="{url_s}" target="_self" style="display:block; text-align:center; background:#00d4ff; color:black; padding:8px; text-decoration:none; border-radius:4px; font-weight:bold;">Consultar Sector</a></div>'
            folium.GeoJson(json.loads(s['geo']), style_function=lambda x: {'fillColor':'#00d4ff','color':'#00d4ff','weight':1,'fillOpacity':0.1},
                           ...
                           tooltip=folium.Tooltip(f"Sector: {s['sector']}"), popup=folium.Popup(pop_s, max_width=250)).add_to(m)

    for id_p, info in mapa_pozos_dict.items():
        d = lambda tag: data_scada.get(tag, (0, "N/A"))
        is_st = (info['status_label'] == 'SIN TELEMETRÍA')
        q, p, sumer, dinam, tanq, col = [d(info[k])[0] if not is_st else 0.0 for k in ['caudal','presion','sumergencia','nivel_dinamico','nivel_tanque','columna']]
        f_q, f_p, f_s, f_d, f_t, f_c = [d(info[k])[1] if not is_st else "N/A" for k in ['caudal','presion','sumergencia','nivel_dinamico','nivel_tanque','columna']]
        h_arr, h_par = formato_hora(d(info['h_arranque'])[0]), formato_hora(d(info['h_paro'])[0])
        v, a = [d(t) for t in info['voltajes_l']], [d(t) for t in info['amperajes_l']]
        ...
        <div style="background:#050505; color:white; padding:15px; border-radius:12px; width:380px; border:1px solid {info['color_final']}; font-family:sans-serif;">
            ...
            <div style="margin-bottom:12px;"><div style="font-size:10px; color:#888;">NIVELES</div>
            ...
            <div style="display:flex; font-size:11px;"><span>▶️ Arranque: <b>{h_arr}</b></span></div>
            <div style="display:flex; font-size:11px;"><span>⏹️ Paro: <b>{h_par}</b></span></div>
        </div>
        """
        if ver_etiquetas: folium.Marker(location=info['coord'], icon=folium.DivIcon(icon_anchor=(-12,10), html=f'<div style="font-size:9px; font-weight:bold; color:{info["color_final"]}; text-shadow:1px 1px #000;">{id_p}</div>')).add_to(m)
        if ver_pozos:
            if info.get('blink'): folium.Marker(location=info['coord'], icon=folium.DivIcon(html=get_blink_icon(info['color_final'])), popup=folium.Popup(html_popup, max_width=450)).add_to(m)
            else: folium.CircleMarker(location=info['coord'], radius=4, color=info['color_final'], fill=True, fill_color=info['color_final'], fill_opacity=1, weight=1, popup=folium.Popup(html_popup, max_width=450)).add_to(m)
