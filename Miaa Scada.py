...
# --- A. CREACIÓN DE COLUMNAS (Mapa a la izquierda, Capas a la derecha) ---
# Usamos una proporción 9:1 para que el panel sea delgado
col_mapa, col_capas = st.columns([9, 1])

with col_capas:
    st.markdown("### 🗺️ Capas")
    # Estos interruptores controlarán el renderizado en tiempo real
...
with col_mapa:
    ...
    # 1. FUNCIÓN PARA HORARIO 00:00 (Tu lógica original intacta)
    ...
    # 2. FUNCIÓN PARA ICONO PARPADEANTE (Tu lógica original intacta)
    ...
    # 3. RENDERIZADO DE POLIGONOS (SECTORES) - Condicionado al sidebar derecho
    if ver_sectores:
        for s in sectores:
            folium.GeoJson(
                json.loads(s['geo']), 
    ...
    # 4. RENDERIZADO DE POZOS (Toda tu lógica de datos Scada y Popups intacta)
    for id_p, info in mapa_pozos_dict.items():
        d = lambda tag: data_scada.get(tag, (0, "N/A"))
        is_st = (info['status_label'] == 'SIN TELEMETRÍA')
        
        # ... (Mantén aquí todos tus cálculos de q, p, sumer, v, a, etc.)
        # ... (Mantén aquí tu variable html_popup completa)

        # CAPA DE TEXTO (Condicionada al sidebar derecho)
        if ver_etiquetas:
            folium.Marker(
                location=info['coord'],
                icon=folium.DivIcon(
        ...
        # CAPA DEL MARCADOR (Condicionada al sidebar derecho)
        if ver_pozos:
            if info.get('blink'):
                folium.Marker(
                    location=info['coord'],
