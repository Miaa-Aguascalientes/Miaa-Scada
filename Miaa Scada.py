...
st.markdown("""
    <style>
        ...
        .label-on { color: #00FF00; font-weight: bold; }
        .label-off { color: #FF0000; font-weight: bold; }
        .label-obs { color: #FF00FF; font-weight: bold; }
        .section-header { padding: 10px; border-radius: 3px; font-weight: bold; margin-bottom: 5px; color: white; }
    </style>
""", unsafe_allow_html=True)

# 3. DICCIONARIO DE CONFIGURACIÓN (Fácil de expandir)
mapa_pozos_dict = {
    "P005A": {
...
# 4. FUNCIONES DE CARGA (Sin cambios significativos)
@st.cache_resource
...
for id_p, info in mapa_pozos_dict.items():
    ...
    if f_bba and (ahora - f_bba).total_seconds() > 14400:
        info['status'] = 'OBSOLETO'
        info['color'] = 'orange'
        pozos_obs.append(id_p)
    elif val_bba == 1:
        info['status'] = 'ENCENDIDO'
        info['color'] = 'green'
...
for id_p, info in mapa_pozos_dict.items():
    ...
    # Popup de alta visibilidad (estilo industrial)
    html_popup = f"""
    <div style="background:#111; color:white; padding:15px; border-radius:10px; width:280px; border:2px solid {info['color']}; font-family:sans-serif;">
        ...
        <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
            <span>ESTADO:</span> <b style="color:{info['color']};">{info['status']}</b>
        </div>
        <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
