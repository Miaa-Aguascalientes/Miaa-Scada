...
# Lógica de clasificación (Respetando el diseño original)
...
# Lógica de navegación de sectores (URL Params)
query_params = st.query_params
if "sector_id" in query_params:
    st.title(f"Vista Detallada: {query_params['sector_id']}")
    st.info("Cargando datos técnicos del sector desde PostgreSQL...")
...
with st.sidebar:
    ...
    with st.expander("🔌 ESTADO DE CONEXIONES", expanded=True):
        ...
        st.markdown(f"**SCADA (MySQL):** {get_tag('OK' if get_mysql_scada_engine() else 'ERROR')}", unsafe_allow_html=True)
        st.markdown(f"**Telemetría (MySQL):** {get_tag('OK' if get_mysql_telemetria_engine() else 'ERROR')}", unsafe_allow_html=True)
        st.markdown(f"**GIS (PostgreSQL):** {get_tag('OK' if get_postgres_conn() else 'ERROR')}", unsafe_allow_html=True)

    if st.button("♻️ Sincronizar Datos Real-Time", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
...
with col_capas:
    ...
    ver_pozos = st.checkbox("Pozos (Iconos)", value=True)
    ver_etiquetas = st.checkbox("Etiquetas ID", value=True)
    st.divider()
    st.info("Haga clic en un pozo para ver la telemetría detallada.")
...
with col_mapa:
    ...
    def get_blink_icon(color):
        return f"""
            ...
            box-shadow: 0 0 10px {color}; animation: blinker 1.2s linear infinite;"></div>
            <style>@keyframes blinker {{ 50% {{ opacity: 0.1; }} }}</style>
        """

    # RENDERIZADO DE SECTORES (GeoJson desde Postgres)
    if ver_sectores:
        for s in sectores:
            try:
                ...
                html_sector = f"""
                    <div style="font-family: sans-serif; color: white; background: #0b1a29; padding: 12px; border-radius: 8px; border: 1px solid #00d4ff; width: 200px;">
                        <h4 style="margin: 0; color: #00d4ff; font-size: 14px;">Sector: {nombre_sector}</h4>
                        <hr style="border: 0.5px solid #333; margin: 8px 0;">
                        <a href="{url_detalle}" target="_self" style="display: block; text-align: center; background: #00d4ff; color: black; padding: 6px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">Ver Detalles Técnicos</a>
                    </div>
                """
                
                folium.GeoJson(
                    json.loads(s['geo']), 
                    style_function=lambda x: {'fillColor': '#00d4ff', 'color': '#00d4ff', 'weight': 1, 'fillOpacity': 0.12},
                    highlight_function=lambda x: {'fillColor': '#00d4ff', 'color': '#ffffff', 'weight': 2, 'fillOpacity': 0.3},
                    popup=folium.Popup(html_sector, max_width=250)
                ).add_to(m)
            except: continue

    # RENDERIZADO DE POZOS (DISEÑO 100% ORIGINAL DEL RESPALDO)
    for id_p, info in mapa_pozos_dict.items():
        d = lambda tag: data_scada.get(tag, (0.0, "N/A"))
        is_st = (info['status_label'] == 'SIN TELEMETRÍA')
        
        # Extracción de valores hidráulicos
        ...
        # HTML del Popup (Tu estructura original de tablas)
        html_popup = f"""
        <div style="background: #050505; color: white; padding: 15px; border-radius: 12px; width: 380px; border: 1px solid {info['color_final']}; font-family: sans-serif;">
            ...
            <div style="margin-bottom: 12px;">
                <div style="font-size: 10px; color: #888; margin-bottom: 4px;">DATOS HIDRÁULICOS</div>
            ...
            <div style="margin-bottom: 12px;">
                ...
                <div style="font-size: 10px; color: #888; border-top: 1px solid #222; padding-top: 5px; margin-bottom: 4px;">HORARIOS DE CONTROL</div>
                <div style="display: flex; align-items: baseline; font-size: 11px; margin-bottom: 3px;">
                    <span>▶️ Próximo Arranque: <b>{h_arr_fmt}</b></span>
                    <span style="color: #FFFF00; font-size: 8px; margin-left: auto;">{f_h_arr}</span>
                </div>
                <div style="display: flex; align-items: baseline; font-size: 11px;">
                    <span>⏹️ Próximo Paro: <b>{h_par_fmt}</b></span>
        ...
        # Capa de Marcadores (Círculos y Blinkers)
        if ver_pozos:
            if info.get('blink'):
                folium.Marker(
                    location=info['coord'],
