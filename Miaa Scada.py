...
# --- LÓGICA DE REDIRECCIÓN DE SECTOR (POSICIONADA PARA NO ROMPER EL MAPA) ---
...
if "sector_id" in query_params:
    ...
    st.info(f"Consultando información técnica de PostgreSQL para {sector_seleccionado}...")
...
with st.sidebar:
    ...
    with st.expander("🔌 CONEXIONES", expanded=True):
        def get_tag(status):
    ...
    st.markdown(f'<div class="resumen-card"><h4 style="color:#00d4ff; margin:0;">RESUMEN</h4><p>Caudal Total: <b style="color:#00FF00;">{total_q:.2f} l/s</b></p></div>', unsafe_allow_html=True)
        ...
        for s in sectores:
            ...
            html_sector = f"""
                <div style="font-family: sans-serif; color: white; background: #0b1a29; padding: 10px; border-radius: 5px; border: 1px solid #00d4ff;">
                    ...
                    <a href="{url_detalle}" target="_self" style="display: block; text-align: center; background: #00d4ff; color: black; padding: 8px; text-decoration: none; border-radius: 4px; font-weight: bold;">Consultar Sector</a>
                </div>"""
            folium.GeoJson(json.loads(s['geo']), style_function=lambda x: {'fillColor': '#00d4ff', 'color': '#00d4ff', 'weight': 1, 'fillOpacity': 0.1},
                           popup=folium.Popup(html_sector, max_width=250)).add_to(m)

    # RENDERIZADO DE POZOS (DISEÑO ORIGINAL 100% RESTAURADO)
    for id_p, info in mapa_pozos_dict.items():
        ...
        if ver_etiquetas:
            folium.Marker(location=info['coord'], icon=folium.DivIcon(icon_size=(150,36), icon_anchor=(-12, 10),
                html=f'<div style="font-size: 9px; font-weight: bold; color: {info["color_final"]}; white-space: nowrap; text-shadow: 1px 1px #000; pointer-events: none;">{id_p}</div>')).add_to(m)
