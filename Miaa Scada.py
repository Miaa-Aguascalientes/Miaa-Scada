...
with st.sidebar:
    ...
    if pozos_falla_com:
        with st.expander(f"🟠 Falla Com. ({len(pozos_falla_com)})"):
            for p in sorted(pozos_falla_com): st.write(f"🟠 {p}")

# 7. MAPA Y PESTAÑAS (MANTENIENDO TU DISEÑO DE POPUP)
...
with tab_mapa:
    ...
    with col_mapa:
        ...
        for id_p, info in mapa_pozos_dict.items():
            ...
            # TU DISEÑO DE POPUP ORIGINAL RESTAURADO ÍNTEGRAMENTE
            html_popup = f"""
            <div style="background: #050505; color: white; padding: 15px; border-radius: 12px; width: 380px; border: 1px solid {info['color_final']}; font-family: sans-serif;">
                ...
                <div style="margin-bottom: 12px;">
                    ...
                    <div style="display: flex; align-items: baseline; font-size: 11px;"><span>▶️ Arranque: <b>{h_arr_fmt}</b></span><span style="color: #FFFF00; font-size: 8px; margin-left: auto;">{f_h_arr}</span></div>
                    <div style="display: flex; align-items: baseline; font-size: 11px;"><span>⏹️ Paro: <b>{h_par_fmt}</b></span><span style="color: #FFFF00; font-size: 8px; margin-left: auto;">{f_h_par}</span></div>
                </div>
            </div>
            """

            if ver_etiquetas:
                folium.Marker(location=info['coord'], icon=folium.DivIcon(icon_size=(150,36), icon_anchor=(-12, 10), html=f'<div style="font-size: 9px; font-weight: bold; color: {info["color_final"]}; text-shadow: 1px 1px #000;">{id_p}</div>')).add_to(m)
...
with tab_info_sectores:
    st.header("📊 Detalle de Sectores")
    if sectores:
        df_sec = pd.DataFrame(sectores)[['sector']]
        st.dataframe(df_sec, use_container_width=True)
    else: st.info("No hay sectores cargados.")
