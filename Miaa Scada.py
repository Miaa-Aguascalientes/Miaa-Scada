...
# 7--------------------------------------------------------------------------------- SECCION 7. DASHBOARD Y PESTAÑAS -------------------------------------------------------------------------------------------------------------
...
with tab_mapa:
    ...
    with col_mapa:
        ...
        for id_p, info in mapa_pozos_dict.items():
            ...
            <div style="background: #050505; color: white; padding: 15px; border-radius: 12px; width: 380px; border: 1px solid {info['color_final']}; font-family: sans-serif;">
                ...
                <div style="margin-bottom: 12px;">
                    ...
                    <div style="display: flex; align-items: baseline; font-size: 11px;">
                        ...
                        <span style="color: #FFFF00; font-size: 8px; margin-left: auto;">{f_h_paro}</span>
                    </div>
                </div>
            </div>
            """
            ...
            if ver_etiquetas:
                folium.Marker(location=info['coord'], icon=folium.DivIcon(icon_size=(150,36), icon_anchor=(-12, 10), html=f'<div style="font-size: 9px; font-weight: bold; color: {info["color_final"]}; white-space: nowrap; text-shadow: 1px 1px #000; pointer-events: none;">{id_p}</div>')).add_to(m)
...
with tab_info_sectores:
    st.markdown("## 📊 Información Detallada de Sectores")
    if sectores:
        df_sec = pd.DataFrame(sectores)[['sector']]
        st.dataframe(df_sec, use_container_width=True, hide_index=True)
    else:
        st.warning("No se pudo cargar la información de sectores.")
