...
st.markdown("""
    <style>
        ...
        .element-container:has(iframe) { margin-top: 10px !important; }
        .titulo-superior {
            position: fixed;
            top: 15px;
            left: 50%;
        ...
        .sidebar-logo { display: flex; justify-content: center; padding: 0px !important; margin-top: -50px !important; margin-bottom: 10px; }
...
if sector_seleccionado:
    ...
    if datos_s:
        def micro_metric(label, value):
            st.markdown(f'<div style="background: #0b1a29; border: 1px solid #1f4068; border-radius: 5px; padding: 8px; text-align: center;"><div style="color: #888; font-size: 10px;">{label}</div><div style="color: #00d4ff; font-size: 15px; font-weight: bold;">{value}</div></div>', unsafe_allow_html=True)
        ...
        for id_p in ids_pozos:
            if id_p in mapa_pozos_dict:
                ...
                html_popup_sec = f"""<div style="background: #050505; color: white; padding: 15px; border-radius: 12px; width: 250px; border: 1px solid {info['color_final']};"><b>POZO {id_p}</b><br>Estado: {info['status_label']}<br>Caudal: {q:.2f} L/s</div>"""

                if info.get('blink'):
                    folium.Marker(location=info['coord'], icon=folium.DivIcon(html=get_blink_icon(info['color_final'])), popup=folium.Popup(html_popup_sec, max_width=450)).add_to(m_sec)
                else:
                    folium.CircleMarker(location=info['coord'], radius=5, color=info['color_final'], fill=True, fill_color=info['color_final'], fill_opacity=1, popup=folium.Popup(html_popup_sec, max_width=450)).add_to(m_sec)
                
                folium.Marker(location=info['coord'], icon=folium.DivIcon(icon_anchor=(-12, 12), html=f'<div style="font-size: 9px; font-weight: bold; color: {info["color_final"]};">{id_p}</div>')).add_to(m_sec)
...
# 8. SIDEBAR (BARRA LATERAL) - DECLARADA ANTES DEL MAPA PRINCIPAL PARA EVITAR PROBLEMAS DE CARGA
with st.sidebar:
    ...
    with st.expander("🔌 Estado de las Conexiones", expanded=False):
        ...
        def render_status_line(label, status):
            ...
            st.markdown(f'<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;"><span style="font-weight: bold; font-size: 13px;">{label}</span><span class="status-tag {cls}">{status}</span></div>', unsafe_allow_html=True)
    ...
    pozo_buscado = st.selectbox("🔍 Localizar Sitio", options=[""] + lista_pozos_nombres, format_func=lambda x: "Seleccionar Sitio..." if x == "" else f" {x}")

    lista_sectores = sorted([s['sector'] for s in sectores])
    sector_buscado = st.selectbox("🏘️ Localizar Sector", options=[""] + lista_sectores, format_func=lambda x: "Seleccionar Sector..." if x == "" else f" {x}", key="busqueda_sectores")
    ...
    # --- CONTROL DE CAPAS (DEFINIDAS ANTES DEL MAPA) ---
