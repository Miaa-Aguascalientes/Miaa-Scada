...
# 4. CARGA DE DATOS (TTL corto para auto-refresh)
...
@st.cache_data(ttl=30)
def cargar_datos_scada_actualizados(mapa_pozos):
...
# --- 5. MONITOR PRINCIPAL (AUTO-REFRESH) ---
@st.fragment(run_every="30s")
def monitor_sistema():
    mapa_pozos_dict = cargar_datos_base()
    data_scada = cargar_datos_scada_actualizados(mapa_pozos_dict)
    sectores = cargar_sectores_estaticos()
    ...
    for id_p, info in mapa_pozos_dict.items():
        ...
        if bomba_val == "Sin telemetria":
            ...
            val_bba, _ = data_scada.get(bomba_val, (0, "N/A"))
    ...
    with st.sidebar:
        ...
        <div class="resumen-card">
            ...
            <p style="font-size: 10px; color: #888;">Última actualización: {datetime.now().strftime('%H:%M:%S')}</p>
    ...
    for id_p, info in mapa_pozos_dict.items():
        ...
        <div style="background: #050505; color: white; padding: 15px; border-radius: 12px; width: 380px; border: 1px solid {info['color_final']}; font-family: sans-serif;">
            ...
            <div style="margin-bottom: 12px;">
                ...
                <table style="width: 100%; font-size: 10px; border-collapse: collapse;">
                    <tr style="color: #00d4ff; border-bottom: 1px solid #333;"><th style="padding:4px;">Fase</th><th>Voltaje</th><th>Amp</th></tr>
        ...
        folium.CircleMarker(
            location=info['coord'], radius=7, color=info['color_final'], fill=True, fill_opacity=1, weight=0,
            class_name="blink_me" if info['blink'] else "", popup=folium.Popup(html_popup, max_width=450)
        ).add_to(m)
        folium.map.Marker(location=info['coord'], icon=folium.DivIcon(html=f'<div style="font-size:14px; font-weight:bold; color:{info["color_final"]};">{id_p}</div>')).add_to(m)
...
with st.sidebar:
    ...
    with st.expander("🔌 ESTADO DE CONEXIONES", expanded=True):
        st.write(f"SCADA: {'OK' if get_mysql_scada_engine() else 'ERROR'}")
        st.write(f"Telemetría: {'OK' if get_mysql_telemetria_engine() else 'ERROR'}")

    if st.button("♻️ Forzar Actualización", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
